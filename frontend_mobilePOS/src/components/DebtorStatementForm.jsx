...
export default function DebtorStatementForm({ onClose, onSave, editData }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 bg-[#f8fbff] rounded-lg p-4 border border-[#b6e0fe] shadow space-y-2 animate-fadein">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        ...
        <input name="description" type="text" placeholder="Description" value={form.description} onChange={handleChange} required className="input" />
        <input name="debit" type="number" placeholder="Debit" value={form.debit} onChange={handleChange} className="input" />
        <input name="credit" type="number" placeholder="Credit" value={form.credit} onChange={handleChange} className="input" />
        <input name="balance" type="number" placeholder="Balance" value={form.balance} onChange={handleChange} className="input" />
      </div>
